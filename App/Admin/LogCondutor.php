...
if(isset($_POST['mes']))
{
    ...
    $a = $log->selecMesCondutor($_POST['mes'],$id);
    $smarty->assign('res',$a);
...
if(isset($_POST['DellLogs']))
{
    ...
    if(isset($_GET['id']))
    {
        $util->ir('./LogCondutor?id='.$util->xss($_GET['id']),$msg,'info');
    }
    else
    {
        $util->ir('./LogCondutor',$msg,'info');
    }
}
elseif(isset($_POST['dellFull']))
{
	$log = new Sistema_Logs();
    $log->dellFullCondutor();
...
$paginacao->setTable('log_condutor');
...
$paginacao->setPagina(PATCH.'/Admin/LogCondutor');
