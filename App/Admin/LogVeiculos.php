...
if(isset($_POST['mes']))
{
    ...
    $a = $log->selecMesVeiculos($_POST['mes'],$id);
    $smarty->assign('res',$a);
    $smarty->assign('Miolo','include_logsCondutor.html');
...
if(isset($_POST['DellLogs']))
{
    ...
    if(isset($_GET['id']))
    {
        $util->ir('./LogVeiculos?id='.$util->xss($_GET['id']),$msg,'info');
    }
    else
    {
        $util->ir('./LogVeiculos',$msg,'info');
    }
}
elseif(isset($_POST['dellFull']))
{
	$log = new Sistema_Logs();
    $log->dellFullVeiculos();
...
$paginacao->setTable('log_veiculo');
...
$paginacao->setPagina(PATCH.'/Admin/LogVeiculos');
...
$smarty->assign('Miolo','include_logsVeiculos.html');
$smarty->assign('Titulo_interna',"Painel Administrativo");
$smarty->assign("Pagina","Painel Administrativo");    
$smarty->assign('Container','include_interna.html');
$smarty->assign('Topo','include_topo.html');
