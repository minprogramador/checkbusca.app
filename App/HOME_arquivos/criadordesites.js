...
var fullImage = function() {
  var init = function() {
    $(document).on('click', 'img.owl-lazy[data-zoom="true"]', enableFullImage);
  },
  ...
  loadImage = function(imageUrl) {
    $('.cs-full-image').on('load', function(){
  ...
  appendOverlay = function() {
    ...
    overlay = $('<div/>', { class: 'cs-overlay cs-is-open' }).append(loading, closeButton)
  ...
  buildInfo = function(event) {
    ...
    }).prepend(titleText).appendTo(infoWrapper);
    ...
    if (imageHref) {
      ...
      }).prepend(linkText).appendTo(infoWrapper);
    }
  ...
  buildIcon = function(icon_name) {
    var icon = '<svg version="1.1" class="cs-ico-close" xmlns="http://www.w3.org/2000/svg" \
...
$(document).ready(parallax.init);
 ...
 var productModal = (function productModal(module){
  ...
  function productGallery(photos) {
    ...
    var src = firstPhoto ? firstPhoto.settings.thumbnails.medium.src : emptyPhotoSrc;
  ...
  function productInformations(product) {
    return '<div class="cs-product-informations">\
  ...
  function productPrice(price, offerPrice) {
    ...
    return 'De <span class="price-discount">R$' + productPrice + '</span> \
      ...
      if(variations.length > 0) {
        ...
        if ($invalidVariation.length <= 0 && variation) {
          ...
          $.event.trigger('addProduct',
                          { productId: productId, variationId: variation.id });
        }
      } else {
        $('.cs-product-action').attr('data-is-loading', true);
        $.event.trigger('addProduct',
  ...
  module.init = function init() {
    if ($('#fb-root').length == 0) {
      ...
      (function(d, s, id) {
        ...
        js.src = 'https://connect.facebook.net/pt_BR/sdk.js#xfbml=1&version=v2.10&appId=425320334277338&autoLogAppEvents=1';
...
$(document).ready(facebookComment.init);
...
function Gallery(dom) {
  ...
  this.setupNumber = function() {
    this.dom.children('.owl-dots')
      .toggleClass('owl-dots-as-number');
    this.dom.parent('.cs-gallery-widget')
...
var csform = function() {
  ...
  setMessage = function($form, isValid, message) {
    ...
    $message.addClass(errorClasses[isValid])
            .text(message).focus();
  },

  clearMessage = function($message) {
    $message.removeClass(errorClasses[0])
