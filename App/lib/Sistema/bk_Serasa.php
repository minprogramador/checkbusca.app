...
class Sistema_Serasa extends Sistema_Db_Abstract
{
    ...
    {
		$id = $this->getConjunto($servico);	
        $db  = $this->getDb();
        $stm = $db->prepare("select * from $this->_table where id=:id and servico=:servico");
	...
	public function getConta($servico)
	{
        ...
        $stm = $db->prepare("select * from 
               $this->_table where id=:id and servico=:servico ORDER BY id DESC");
    ...
    public function Logar($servico=null)
    {
        ...
        $LToken   = $result['token'];
        $resNew = $util->curl("https://sitenet.serasa.com.br/novoconcentre/ConcentrePrincipal?param=$LToken",$Lcookie,null,true);
		
        $ver = explode('CPF :',$resNew);

        if(isset($ver[1]) == true)
        {
            $this->setCookie($Lcookie);
            $this->setNcookie($Lcookie);
        ...
        else
        {
			...
			if(preg_match("/var p = \"(.*?)\"/si",$logar, $matches))
            {
				...
				if($this->Verific($this->getNcookie()) == true)
            	{
                	...
                	$db = $this->getDb();
					$stm = $db->prepare('update ' . $this->_table . ' set cookie=:cookie, token=:token where id=:id');
					$stm->bindValue(':id', $idUser);
